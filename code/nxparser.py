...
class NXParser:
    ...
    def get_current_headnodes(self):
        ...
        headnodes_list=[element for element in possible_headnodes_list if (element in self.mesh_nx)] 
        return headnodes_list       
    ...
    def make_mesh_nx(self,mesh_file_address):
        with open(mesh_file_address, "r") as infile:
            ...
            for entry in self.readMeSH(infile):
                self.add_nodepath_and_label_to_endnode_to_mesh_networkx(self.mesh_nx,entry)

        self.current_headnodes=self.get_current_headnodes()
    ...
    def complete_top_of_nx(self):
        ...
        temp_edges=list()
        for element in self.headnodes_list:
            temp_edges.append(
                (element[0],element)
            )
        #print(temp_edges)
    ...
    def remove_mesh_subgraphs(self,headnodes_list):
        for temp_headnode in headnodes_list:
            nodes_to_remove=[
